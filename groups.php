...
if(SESSION) $MyGroups = Groups::GetUserGroups(SESSION["userId"]);
...
else
{
	...
	else $GroupInfo = Groups::GetLastGroupUserJoined(SESSION["userId"]);
}

if($HasGroups)
{
	$GroupsCount = SESSION ? $MyGroups->rowCount() : 0;
	$Emblem = Thumbnails::GetAssetFromID($GroupInfo->emblem, 420, 420);
	$Status = Groups::GetGroupStatus($GroupInfo->id);
	$Ranks = Groups::GetGroupRanks($GroupInfo->id);
	$MyRank = Groups::GetUserRank(SESSION["userId"] ?? 0, $GroupInfo->id);
	...
	pageBuilder::$pageConfig["title"] = Polygon::FilterText($GroupInfo->name).", a Group by ".$GroupInfo->ownername;
	pageBuilder::$pageConfig["og:description"] = Polygon::FilterText($GroupInfo->description);
	pageBuilder::$pageConfig["og:image"] = $Emblem;
}
else
{
	pageBuilder::$pageConfig["title"] = "My Groups";
}
		...
		<div class="card bg-cardpanel">
			<div class="card-body p-2">
				...
				<div class="d-inline-block" style="padding:2px">
					<a href="/groups?gid=<?=$Group->id?>"><img src="<?=Thumbnails::GetAssetFromID($Group->emblem, 420, 420)?>" width="64" class="img-fluid" title="<?=Polygon::FilterText($Group->name)?>" data-toggle="tooltip" data-placement="right"></a>
				</div>
				<?php } ?>
			</div>
		</div>
						...
						<div class="template d-none">
							<div class="item col-md-2 col-sm-3 col-4 px-2 my-2 text-center">
								<div class="card hover">
									<a href="/user?ID=$UserID"><img class="card-img-top img-fluid" src="<?=Thumbnails::GetStatus("rendering", 250, 250)?>" preload-src="$Avatar" title="$UserName" alt="$UserName"></a>
						...
						<div class="template d-none">
							<div class="item col-md-2 col-sm-3 col-4 px-2 my-2">
								<div class="card info hover">
								    <a href="/groups?gid=$ID"><img src="<?=Thumbnails::GetStatus("rendering", 420, 420)?>" preload-src="$Emblem" class="card-img-top img-fluid p-2" title="$Name" alt="$Name"></a>
				...
				<div class="card">
					...
					<div class="card-body p-0">
						...
						<div class="wall-container">
							...
							<div class="template d-none">
								<div class="item item-striped p-3">
									<div class="row">
										<div class="col-md-2 col-3">
											<img src="<?=Thumbnails::GetStatus("rendering", 420, 420)?> "preload-src="$avatar" class="img-fluid">
										</div>
	...
	<div class="col-xl-2 col-lg-3 mb-4">
		...
		<div class="card bg-cardpanel mb-4">
			<div class="card-body py-2">
				...
				<?php if($GroupInfo->owner != SESSION["userId"]) { ?><button class="btn btn-sm btn-light btn-block leave-group-prompt">Leave Group</button><?php } ?>
