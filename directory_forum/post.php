...
if($_SERVER['REQUEST_METHOD'] == "POST")
{
	...
	if(!$errors["subject"] && !$errors["body"] && !$errors["general"])
	{
		if($threadInfo)
		{
			...
			header("Location: /forum/thread?ID=".$threadInfo->id);
		}
		else
		{
			...
			header("Location: /forum/thread?ID=".$query->fetchColumn());
		}
	}
}
...
<nav aria-label="breadcrumb">
  <ol class="breadcrumb">
    ...
    <?php if($threadInfo) { ?>
    <li class="breadcrumb-item active"><a href="/forum/thread?ID=<?=$threadInfo->id?>"><?=htmlspecialchars($threadInfo->subject)?></a></li>
	...
	<div class="col-md-9">
		<form method="post">
			...
			<div class="form-group row">
			    ...
			    <div class="col-sm-10">
			      <textarea type="text" class="form-control<?=$errors["body"]?' is-invalid':''?>" id="body" name="body" placeholder="10,000 characters max" rows="6"><?=$body?></textarea>
			      <div class="invalid-feedback">
			        <?=$errors["body"]?>
			      </div>
			    </div>
