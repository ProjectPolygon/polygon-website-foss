...
if(isset($_GET['ticket']))
{
	$query = $pdo->prepare("
		...
		WHERE client_sessions.ticket = :ticket AND selfhosted_servers.ping+35 > UNIX_TIMESTAMP() AND NOT used");
	...
	if($sessionInfo)
	{
		...
		db::run("UPDATE client_sessions SET used = 1 WHERE ticket = :ticket", [":ticket" => $_GET['ticket']]);
	}
}
