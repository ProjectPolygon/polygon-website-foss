...
if(isset($_GET['ID']) || isset($_GET['id']))
{
	...
	if(!$info || $moderation && !$isModerator) PageBuilder::instance()->errorCode(404);
	$selfProfile = false;
...
[
	...
	"posts" => $info->ForumThreads + $info->ForumReplies,
	"joined" => date("F j Y", $info->jointime)
];

if(SESSION) $friendship = Users::CheckIfFriends(SESSION["user"]["id"], $info->id);

$pageBuilder = new PageBuilder(["title" => $info->username]);
$pageBuilder->addAppAttribute("data-user-id", $info->id);
$pageBuilder->addAppAttribute("data-self-profile", $selfProfile ? "true" : "false");
...
if(SESSION) 
{
	$pageBuilder->addResource("scripts", "http://ajax.googleapis.com/ajax/libs/jqueryui/1.9.2/jquery-ui.min.js");
...
$pageBuilder->addResource("polygonScripts", "/js/polygon/profile.js");
...
?>
<?php if($moderation) { ?><div class="alert alert-danger px-2 py-1 mb-3" role="alert">This user has been suspended by <?=Users::GetNameFromID($moderation->bannerId)?>. Reason: "<?=$moderation->reason?>"</div><?php } ?>
<div class="row">
	<div class="col-md-6 p-0 left-bank divider-right">
		<div class="p-4">
			...
			<div class="thumbnail-holder text-right" data-reset-enabled-every-page="" data-3d-thumbs-enabled="" data-url="<?=Thumbnails::GetAvatar($info->id)?>">
				<span class="thumbnail-span mx-auto d-block" style="max-width:315px" data-3d-url="/avatar-thumbnail-3d/json?userId=<?=$info->id?>">
					<img alt="<?=$info->username?>" class="img-fluid" src="<?=Thumbnails::GetStatus("rendering")?>" data-src="<?=Thumbnails::GetAvatar($info->id)?>">
				</span>
	...
	<div class="col-md-6 p-0 right-bank">
		<?php if(SESSION) { ?>
		<div class="user-places-container p-4">
			...
			<div class="template d-none">
				...
				<div class="accordion-body px-4">
					...
					<button class="btn btn-success px-4 pt-1 pb-0 mb-3 VisitButton VisitButtonSolo" placeid="$PlaceID" placeversion="$Version" title="Visit this game Solo" data-toggle="tooltip" data-placement="bottom" ><h5 class="font-weight-normal pb-0">Visit</h5></button>
					<button class="btn btn-success px-4 pt-1 pb-0 mb-3 VisitButton VisitButtonEdit" placeid="$PlaceID" placeversion="$Version" title="Open in Studio Mode" data-toggle="tooltip" data-placement="bottom" ><h5 class="font-weight-normal pb-0">Edit</h5></button>
				</div>
			</div>
		</div>
		<div class="divider-top"></div>
