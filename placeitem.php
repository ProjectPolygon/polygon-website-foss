...
if (!$Asset) PageBuilder::errorCode(404);
...
PageBuilder::$Config['title'] = Polygon::FilterText($Asset->name).", ".vowel(Catalog::GetTypeByNum($Asset->type))." by ".$Asset->username;
PageBuilder::$Config["AppAttributes"]["data-asset-id"] = $Asset->id; 
PageBuilder::$Config["AppAttributes"]["data-owns-asset"] = $CanConfigure ? "true" : "false";
...
if (Polygon::IsEmbed())
{
	...
	"<div class=\"text-center\"><h1>wtf are you doing</h1></div>";
...
PageBuilder::AddResource(PageBuilder::$PolygonScripts, "/js/polygon/item.js");
if($IsStaff) PageBuilder::AddResource(PageBuilder::$PolygonScripts, "/js/polygon/admin/asset-moderation.js");
...
<div class="container" style="max-width: 58rem">
	...
	<div class="row">
		...
		<div class="col-sm-4">
			...
			<?php if ($Asset->publicDomain || $IsCreator) { ?>
			<button class="btn btn-success btn-block my-2 pt-1 pb-0 mx-auto VisitButton VisitButtonSolo" placeid="<?=$Asset->id?>" placeversion="<?=$Asset->Version?>" style="max-width: 13rem"><h5 class="font-weight-normal pb-0">Visit</h5></button>
			<button class="btn btn-success btn-block my-2 pt-1 pb-0 mx-auto VisitButton VisitButtonEdit" title="Open in Studio Mode" data-toggle="tooltip" data-placement="bottom" placeid="<?=$Asset->id?>" placeversion="<?=$Asset->Version?>" style="max-width: 13rem"><h5 class="font-weight-normal pb-0">Edit</h5></button>
			<?php } ?>
			<div class="details mt-3" style="line-height: normal">
				...
				<i class="<?=Catalog::$GearAttributesDisplay[$Attribute]["icon"]?>" data-toggle="tooltip" data-placement="bottom" title="<?=Catalog::$GearAttributesDisplay[$Attribute]["text_sel"]?>"></i>
				<?php } } if ($IsCreator || $IsStaff) { ?> 
				<div class="divider-bottom mt-3 mb-2"></div>
				<p class="mb-2"><small><b>Options</b></small></p>
				<?php if ($CanConfigure) { ?>
				<p class="mb-0"><small><a href="/places/<?=$Asset->id?>/update">Configure this Place</a></small></p>
				<?php } if($IsAdmin) { ?>
				<p class="mb-0"><small><a class="asset-<?=$Asset->approved==1?'decline':'approve'?>" href="#"><?=$Asset->approved==1?'Disa':'A'?>pprove this Place</a></small></p>
				<p class="mb-0"><small><a class="asset-rerender" href="#">Re-render this Place</a></small></p>
				<?php } } ?>
			</div>
		</div>
	</div>
