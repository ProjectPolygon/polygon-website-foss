...
if($_SERVER["REQUEST_METHOD"] == "POST")
{
	...
	// if(SESSION["currency"] < 500) $Errors->General = "You do not have the sufficient funds to create a group";
	...
	$CreatedGroups = db::run("SELECT COUNT(*) FROM groups WHERE owner = :UserID", [":UserID" => SESSION["userId"]])->fetchColumn();
	if($CreatedGroups >= 3) $Errors->General = "You can only create a maximum of three groups";

	if(Groups::GetUserGroups(SESSION["userId"])->rowCount() >= 20) $Errors->General = "You have reached the maximum number of groups";
	...
	if(!$Errors->Name && !$Errors->Description && !$Errors->Entry && !$Errors->Emblem && !$Errors->General)
	{
		...
		$EmblemID = Catalog::CreateAsset(["type" => 22, "creator" => SESSION["userId"], "name" => $Fields->Name, "description" => "Group Emblem"]);
		$Processor = Image::Process($Image, ["name" => "$EmblemID", "resize" => false, "dir" => "/asset/files/"]);
		...
		if(!$Errors->Emblem)
		{
			...
			db::run(
				...
				[":UserID" => SESSION["userId"], ":EmblemID" => $EmblemID, ":Name" => $Fields->Name, ":Description" => $Fields->Description, ":Entry" => $Fields->Entry]
			);
