...
if (!empty($JobTicket))
{
	$JobSession = Database::singleton()->run(
		...
		WHERE Ticket = :JobTicket AND GameJobs.Status = \"Ready\" AND NOT Verified AND GameJobSessions.TimeCreated + 300 > UNIX_TIMESTAMP()", 
		[":JobTicket" => $JobTicket]
	)->fetch(\PDO::FETCH_OBJ);
	...
	if ($JobSession !== false)
	{
		...
		$Parameters->PingURL = "http://{$_SERVER['HTTP_HOST']}/Game/ClientPresence.ashx?PlaceID={$Parameters->PlaceID}";

		if (is_null($JobSession->ReplicatorTicket))
		{
			// 2010 tickets are formatted as '{date};{installRemotePlayerTicket}'
			// 2011 and newer tickets are formatted as '{date};{installRemotePlayerTicket};{preauthenticateTicket}'
			// so here we'll have to account for both
			// if i can retrofit replicator tickets into 2012 then i might be able to retrofit preauthenticateTicket into 2010

			$date = date('n/j/Y g:i:s A');

			$installRemotePlayerTicket = RBXClient::CryptGetSignature(sprintf(
				"%s\n%s\n%s\n%s\n%s", 
				$Parameters->UserID, 
				$Parameters->Username, 
				$Parameters->CharacterAppearance, 
				$Parameters->JobID, 
				$date
			));

			if ($JobSession->Version == 2010)
			{
				$Parameters->ReplicatorTicket = sprintf("%s;%s", $date, $installRemotePlayerTicket);
			}
			else
			{
				$preauthenticateTicket = RBXClient::CryptGetSignature(sprintf(
					"%s\n%s\n%s", 
					$Parameters->UserID, 
					$Parameters->JobID, 
					$date
				...
				$Parameters->ReplicatorTicket = sprintf("%s;%s;%s", $date, $installRemotePlayerTicket, $preauthenticateTicket);
			}

			Database::singleton()->run(
				"UPDATE GameJobSessions SET ReplicatorTicket = :ReplicatorTicket WHERE Ticket = :JobTicket",
				[":ReplicatorTicket" => $Parameters->ReplicatorTicket, ":JobTicket" => $JobTicket]
			);
		}
		else
		{
			$Parameters->ReplicatorTicket = $JobSession->ReplicatorTicket;
		}
...
function onConnectionAccepted(url, replicator)
	...
	local success, err = pcall(function()	
		if not test then 
		    visit:SetPing("<?=$Parameters->PingURL?>", 60) 
		end
...
local success, err = pcall(function()	
	...
	client.Ticket = "<?=$Parameters->ReplicatorTicket?>"	
