...
if($_SERVER['REQUEST_METHOD'] == 'POST')
{
	...
	else if (!$userInfo) $errors["username"] = "That user doesn't exist";
	else if (!$auth->verify($userInfo->password)) $errors["password"] = "Incorrect password";
	else if (Polygon::IsDevSite() && !in_array($userInfo->id, SITE_CONFIG["DevWhitelist"])) $errors["password"] = "An unexpected error occurred";

	if(!$errors["username"] && !$errors["password"])
	{
		// upgrade password to argon2id w/ encryption if still using bcrypt
		if(strpos($userInfo->password, "$2y$10") !== false) $auth->update($userInfo->id);

		Session::Create($userInfo->id);
...
$pageBuilder = new PageBuilder(["title" => "Login", "ShowNavbar" => !$studio]);
