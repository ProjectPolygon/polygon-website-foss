...
if($_SERVER['REQUEST_METHOD'] == "POST")
{
	...
	if($modelId)
	{
		$query = Database::singleton()->run("SELECT type FROM assets WHERE id = :id AND creator = :uid", [":id" => $modelId, ":uid" => $userid]);
	...
	if(strlen($xml) > 16000000) error("Model cannot be larger than 16 megabytes");
	...
	if($modelId) 
		unlink(Polygon::GetSharedResource("assets/{$modelId}"));
	else
		$modelId = Catalog::CreateAsset([
			...
			"description" => $desc, 
			"sale" => (int)$free, 
			"PublicDomain" => (int)$free, 
			"approved" => (int)$isScript
		]);

	file_put_contents(Polygon::GetSharedResource("assets/{$modelId}"), $xml);
...
$models = Database::singleton()->run("SELECT * from assets WHERE creator = :uid AND type = 10 ORDER BY created DESC", [":uid" => $userid]);
?>
