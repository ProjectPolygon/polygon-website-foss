...
if($_SERVER['REQUEST_METHOD'] == 'POST')
{
	...
	else
	{
		...
		if($file && $file["size"])
		{
			if($isAdmin && !in_array($item->type, [1, 3, 10]))
			{
		...
		<div class="card mb-3">
			<div class="card-header py-2">Update asset data <a href="/asset/?id=<?=$item->id?>" class="float-right">Download</a></div>
		  	<div class="card-body">
		  		...
		  		<span>Preview:</span>
		  		<textarea class="form-control" style="resize:none" rows="12" tabindex="2" disabled="disabled"><?=trim($item->type == 10 ? Gzip::Decompress($_SERVER['DOCUMENT_ROOT']."/asset/files/".$item->id) : file_get_contents($_SERVER['DOCUMENT_ROOT']."/asset/files/".$item->id))?></textarea>
		 	</div>
		</div>
		<?php } if($item->type == 3) { ?>
		<div class="card mb-3">
			<div class="card-header py-2">Set thumbnail</div>
		  	<div class="card-body">
		  		<input type="file" class="form-control-file form-control-sm mb-4" id="file" name="file">
		  		<span>Note: thumbnail should be in a 1:1 aspect ratio, like an album cover</span>
		 	</div>
		</div>
		<?php } ?>
		<div class="card mb-3">
