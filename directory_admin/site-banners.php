...
if($_SERVER['REQUEST_METHOD'] == "POST")
{
  ...
  if($mode == "create")
  {
    ...
    elseif(Database::singleton()->run("SELECT COUNT(*) FROM announcements WHERE activated")->fetchColumn() > 5) $error = "There's too many banners currently active!";
    else
    {
      Database::singleton()->run(
        "INSERT INTO announcements (createdBy, text, bgcolor, textcolor) VALUES (:uid, :text, :bgc, :tc)",
        [":uid" => SESSION["user"]["id"], ":text" => $text, ":bgc" => $backcolor, ":tc" => $textcolor]
      );
  ...
  else//if($mode == "delete")
  {
    ...
    Database::singleton()->run("UPDATE announcements SET activated = 0 WHERE id = :id", [":id" => $id]);
  }

  Polygon::GetAnnouncements();
}

$pageBuilder = new PageBuilder(["title" => "Site banners"]);
$pageBuilder->addResource("stylesheets", "/css/bootstrap-colorpicker.min.css");
$pageBuilder->addResource("scripts", "/js/bootstrap-colorpicker.min.js");
$pageBuilder->addResource("scripts", "https://cdnjs.cloudflare.com/ajax/libs/markdown-it/11.0.1/markdown-it.min.js");
