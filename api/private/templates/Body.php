...
<?php if (SESSION) { ?>
						...
						<div class="navbar-button-container">
							<a class="btn btn-sm btn-light py-0 px-1 friend-requests-indicator<?= !SESSION["user"]["PendingFriendRequests"]?' d-none':'' ?>"><?= SESSION["user"]["PendingFriendRequests"] ?></a>
			...
			<div class="container">
				...
				<div class="collapse navbar-collapse" id="secondaryNavbar">
					<ul class="navbar-nav mr-auto">
						...
						<li class="nav-item">
							<a class="nav-link py-1" href="/my/invites">Invitations</a>
						</li>
<?php if (SESSION && SESSION["user"]["adminlevel"]) { ?>
						<li class="nav-item">
							<a class="nav-link py-1" href="/admin">Admin <span class="btn btn-sm btn-outline-light py-0<?= $this->templateVariables["PendingAssets"] == 0 ? ' d-none' : '' ?>" style="margin-top:-3px"><?= $this->templateVariables["PendingAssets"] ?></span></a>
						</li>
<?php } ?>
					</ul>
				</div>
...
<?php foreach ($this->templateVariables["Announcements"] as $announcement) { ?>
		<div class="alert py-2 mb-0 rounded-0 text-center text-<?= $announcement["textcolor"] ?>" role="alert" style="background-color: <?= $announcement["bgcolor"] ?>">
			<?= $this->templateVariables["Markdown"]->text($announcement["text"], true) ?>
		</div>
<?php } /* foreach ($announcements as $announcement) */ ?>
<?php } /* endif ($this->config["ShowNavbar"]) */ ?>
