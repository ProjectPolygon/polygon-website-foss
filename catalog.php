...
$query = Database::singleton()->run(
	...
	[":keywd" => "%{$keyword}%", ":offset" => $pagination->Offset]
);

$pageBuilder = new PageBuilder(["title" => "Avatar Items, Virtual Avatars, Virtual Goods"]);
$pageBuilder->addResource("polygonScripts", "/js/polygon/catalog.js");
		...
		<div class="catalog-container">
			...
			<div class="row">
				<div class="col-xl-9 col-lg-8 col-md-6">
					<p>Showing <?=number_format($pagination->Offset+1)?> - <?=number_format($pagination->Offset+$query->rowCount())?> of <?=number_format($results)?> results</p>
				</div>
				...
				<div class="item col-xl-2 col-lg-3 col-md-3 col-sm-4 col-6 pb-3 px-2" style="line-height:normal">
					<div class="card info hover">
						...
						<div class="card-body p-2">
						  	...
						  	<?php if($item->sale) { ?><p class="m-0 text-success"><?=$item->price ? '<i class="fal fa-pizza-slice"></i> '.number_format($item->price):'Free'?></p><?php } ?>
						</div>
					</div>
			...
			<div class="pagination form-inline justify-content-center">
				...
				<button type="button" class="btn btn-light mx-3 next"<?=$page>=$pagination->Pages?' disabled':''?>><h5 class="mb-0"><i class="fal fa-caret-right"></i></h5></button>
			</div>
			<?php } ?>
		</div>
	</div>
