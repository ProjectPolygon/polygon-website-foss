...
$threadInfo = forum::getThreadInfo($_GET['ID'] ?? $_GET['id'] ?? false);
...
<div class="row mb-2">
	<div class="col">
		<?php if(SESSION){ ?><a class="btn btn-primary<?=$threadInfo->deleted?' disabled':''?>" href="/forum/post?thread=<?=$threadInfo->id?>">New Reply</a><?php } ?>
		...
		<nav aria-label="Forum thread pagination">
		  <ul class="pagination justify-content-end mb-0">
		    <li class="page-item<?=$page<=1?' disabled':''?>">
		      <a class="page-link" <?=$page>1?'href="/forum/thread?ID='.$threadInfo->id.'&page='.($page-1).'"':''?>aria-label="Previous">
		        <span aria-hidden="true">&laquo;</span>
		        <span class="sr-only">Previous</span>
		      </a>
		    </li>
		    <?php $paginator = 1; while($paginator <= $pages){ ?>
		    <li class="page-item<?=($paginator == $page)?' active':''?>"><a class="page-link" href="/forum/thread?ID=<?=$threadInfo->id?>&page=<?=$paginator?>"><?=$paginator?></a></li>
			<?php $paginator++; } ?>
		    <li class="page-item<?=$page>=$pages?' disabled':''?>">
		      <a class="page-link" <?=$page<$pages?'href="/forum/thread?ID='.$threadInfo->id.'&page='.($page+1).'"':''?>aria-label="Next">
...
<div class="card-body">
	<div class="row">
		<div class="col-md-2 divider-right">
			...
			<img src="/thumbs/avatar?id=<?=$threadInfo->author?>&x=180&y=220" title="<?=$authorInfo->username?>" alt="<?=$authorInfo->username?>" width="135" height="135" class="img-fluid mt-3">
