...
if ($_SERVER['REQUEST_METHOD'] == "POST")
{
	...
	if($PlaceID)
	{
		$PlaceCheck = Database::singleton()->run(
	...
	if (strlen($xml) > 32000000) error("Place cannot be larger than 32 megabytes");
	...
	$xml = str_ireplace("http://".$_SERVER['HTTP_HOST']."/asset?id=", "%ASSETURL%", $xml);
	$xml = preg_replace("/rbxasset:\/\/..\/[^<]*/", "", $xml);

	if ($PlaceID) 
	{
		unlink(Polygon::GetSharedResource("assets/{$PlaceID}"));
		Database::singleton()->run("UPDATE assets SET updated = UNIX_TIMESTAMP() WHERE id = :PlaceID", [":PlaceID" => $PlaceID]);
	}
	else
	{
