...
$MyRank = Groups::GetUserRank(SESSION["user"]["id"], $GroupInfo->id);
if(!$MyRank->Permissions->CanManageGroupAdmin) PageBuilder::instance()->errorCode(404);
...
if($_SERVER["REQUEST_METHOD"] == "POST" && $MyRank->Level == 255)
{
	...
	$GroupExists = Database::singleton()->run("SELECT COUNT(*) FROM groups WHERE name = :Name", [":Name" => $Fields->Name])->fetchColumn();
	...
	if(!$Errors->Name && !$Errors->Description)
	{
		if($Emblem && $Emblem["size"] !== 0)
		{
			...
			if($Image->uploaded) 
			{
				...
				$Processed = Image::Process($Image, ["name" => $GroupInfo->emblem, "keepRatio" => true, "align" => "T", "x" => 128, "y" => 128, "dir" => "assets/"]);
				...
				if ($Processed === true) 
				{
					...
					Database::singleton()->run("UPDATE assets SET approved = 0 WHERE id = :EmblemID", [":EmblemID" => $GroupInfo->emblem]);
				}
				else
				{
					$Errors->Emblem = "Image processing failed: $Processed";
				}
			}
			else
			{
				$Errors->Emblem = "Failed to upload image";
			}
		}
		...
		if($GroupInfo->name != $Fields->Name)
		{
			Groups::LogAction(
				...
				sprintf(
					...
					SESSION["user"]["id"], SESSION["user"]["username"], htmlspecialchars($Fields->Name)
				)
			);
		}
		...
		if($GroupInfo->description != $Fields->Description)
		{
			Groups::LogAction(
				...
				sprintf(
					...
					SESSION["user"]["id"], SESSION["user"]["username"], htmlspecialchars($Fields->Description)
				)
			);
		}
		...
		<div class="tab-content" id="groupTabsContent">
				...
				<div class="template d-none">
					<div class="item col-md-2 col-sm-3 col-4 px-2 my-2 text-center">
						<div class="card hover">
							<a href="/user?ID=$UserID"><img class="card-img-top img-fluid" data-src="$Avatar" title="$UserName" alt="$UserName"></a>
				...
				<form method="post" enctype="multipart/form-data">
					...
					<div class="row">
						<div class="col-md-3 mb-4">
							...
							<img src="<?=Thumbnails::GetAssetFromID($GroupInfo->emblem)?>" class="img-fluid mb-4">
			...
			<div class="tab-pane" id="relationships" role="tabpanel" aria-labelledby="relationships-tab">
				...
				<div class="allies-container">
					...
					<div class="template d-none">
						<div class="col-md-2 col-sm-3 col-4 px-2 my-2">
							<div class="card hover" data-group-id="$ID">
							    <a href="/groups?gid=$ID"><img data-src="$Emblem" class="card-img-top img-fluid p-2" title="$Name" alt="$Name"></a>
